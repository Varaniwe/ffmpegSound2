...
bool AudioGrabber::init_input(std::string device_name, std::string format_short_name)
{
	...
	char *device_utf8_string = dup_wchar_to_utf8((wchar_t*)device_w_string.c_str());

	int ret = avformat_open_input(&in_fmt_context, device_utf8_string, in_format, NULL);
	if (ret < 0)
	{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					// TODO: можно сделать транскодинг
...
void AudioGrabber::grab()
{
	...
	while (av_read_frame(in_fmt_context, in_pkt) >= 0 && !stop_flag)
	{
		if (in_pkt->stream_index == in_aud_strm_idx)
		{
			...
			{
				// TODO: мерить продолжительность аудио, затем записать
